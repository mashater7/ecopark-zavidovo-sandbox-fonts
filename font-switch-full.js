/* Переключатель шрифтов (ПОЛНЫЙ): применяет выбранный шрифт КО ВСЕМУ тексту — и к обычному, и к заголовкам. */
(function () {
  ...
  // На заголовках это особенно важно — они большие, лишняя строка сразу ломает экран.
  ...
  // ПОЛНЫЙ сбор: все элементы с собственным текстом, БЕЗ порога по размеру —
  // значит, заголовки тоже попадают под замену.
  function collect() {
  ...
  function applyFont(f) {
    ...
    doApply(f);
    // Шрифт мог ещё не догрузиться: тогда ширину мы измерили по запасному шрифту,
    // компенсация неверная. Дожидаемся настоящего шрифта и пересчитываем. Убирает гонку.
  ...
  function buildUI() {
    ...
    var t = document.createElement('span'); t.className = 'fs-title'; t.textContent = 'Шрифт (всё):'; bar.appendChild(t);
    ...
    note.textContent = 'меняет ВЕСЬ текст + заголовки';
